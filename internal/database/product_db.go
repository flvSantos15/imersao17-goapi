...
import (
	...
	"github.com/flvsantos15/imersao17/goapi/internal/entity"
)
...
func (pd *ProductDB) CreateProduct(product *entity.Product) (*entity.Product, error) {
	_, err := pd.db.Exec("INSERT into product (id, name, description, price, categoryID, imageURL) VALUES(?, ?, ?, ?, ?, ?)", product.ID, product.Name, product.Description, product.Price, product.CategoryID, product.ImageURL)
	if err != nil {
		return nil, err
	}
	return product, nil
